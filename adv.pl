...
nshelp:-
  ...
  write('   throw_item something  (ex. throw_item apple'),nl,
  write('   eat something         (ex. eat the apple)'),nl,
  write('   turn something on     (ex. turn_on the light)'),nl,
...
take(X):-
  heavy(X),
  respond(['That is too heavy to take']),
  !.
...
% throw_item - allows a player to throw away a item out of window

throw_item(Thing):-
  have(Thing),
  retract(have(Thing)),
  asserta(location(Thing,garden)),
  respond(['You have throw away ', Thing]).

throw_item(Thing):-
  respond(['You do not have ', Thing, ' to throw']),
  !.
...
can_take(Thing) :-
  have(Thing),
  respond(['You already took ',Thing]),
  nl,
  !.
...
tran_verb(throw_item) --> [throw,item].
tran_verb(eat) --> [eat].
